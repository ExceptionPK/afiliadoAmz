...
const PushPermissionPrompt = ({ session }) => {
  ...
  useEffect(() => {
    ...
    const checkPromptStatus = async () => {
      try {
        // 1. Obtener preferencia guardada de ESTE usuario
        ...
        // 2. Estado actual de OneSignal / navegador
        const permission = await OneSignal.Notifications.permission;
        const isSubscribed = await OneSignal.User.PushSubscription.optedIn;

        // Mostrar prompt SOLO si:
        // - El usuario aún NO ha activado push en nuestra BD
        // - No está suscrito actualmente en OneSignal
        if (!userWantsPush && !isSubscribed) {
          console.log(`🔔 Mostrando prompt para usuario ${session.user.id}`);
          setTimeout(() => {
            setShow(true);
            setTimeout(() => setVisible(true), 200);
          }, 1800);
        } else {
          console.log(`⏭️ Prompt omitido para usuario ${session.user.id} (ya activado o suscrito)`);
        }
      } catch (err) {
        console.error("Error checking push prompt status:", err);
      }
    };

    // Esperamos un poco a que OneSignal esté listo
    const timer = setTimeout(checkPromptStatus, 1000);
