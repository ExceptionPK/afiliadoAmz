            ...
            <div className="min-h-screen">
                <div className="relative z-10 container mx-auto px-0 py-[200px] md:py-40">
                    ...
                    <div className="text-center mb-2 md:mb-2 opacity-0 animate-fade-in-down" style={{ animationDelay: "0.3s" }}>
                        <button
                            onClick={(e) => { e.preventDefault(); e.stopPropagation(); openInRealBrowser("https://www.amazon.es"); }}
                        ...
                        >
                            <img src="../amazon.svg" alt="Amazon" className="h-20 md:h-20 lg:h-20 amazon-morado amazon-glass" />
                        </button>
                    </div>
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        />
                                        {inputUrl && (
                                            <button onClick={handleReset} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 transition">
                                ...
                                {/* Error */}
                                <div className={`overflow-hidden transition-all duration-300 ${showError ? "max-h-40 opacity-100" : "max-h-0 opacity-0"}`}>
                                    {error && (
                                        <div className="relative p-4 bg-red-50 border border-red-100 contenedorCosas animate-fade-in">
                                            ...
                                            <p className="text-sm text-red-700 text-center pr-8 pl-12">{error}</p>
                                        </div>
                                    )}
                                </div>

                                {/* Botón generar */}
                                <Button onClick={generateAffiliateLink} disabled={!inputUrl} className="boton-gradiente w-full transition-all duration-300">
                                    Guardar producto
                                </Button>

                                {/* Separador mágico */}
                                <div className={`overflow-hidden transition-all duration-700 ${showResult ? "max-h-32 opacity-100" : "max-h-0 opacity-0"}`}>
                                    ...
                                    {affiliateUrl && (
                                        <div className="space-y-4 -mt-3 animate-fade-in">
                                            <div className="space-y-2">
                                                ...
                                                <div className="relative">
                                                    <Input value={affiliateUrl} readOnly className="pr-32 input-affiliate2 hidden md:block" />
                                                    <Input value={affiliateUrl} readOnly className="pr-32 input-affiliate2 block md:hidden truncate" style={{ paddingRight: '80px' }} />

                                                    <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
                                                        <button onClick={copyToClipboard} className={`p-2 contenedorCosas transition-all ${copied ? "bg-green-100 text-green-600" : "bg-slate-100 hover:bg-slate-200 text-slate-600"}`} title={copied ? "¡Copiado!" : "Copiar enlace"}>
                                                            {copied ? <CheckCircle2 className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                                                        </button>
                                                        <button onClick={() => window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(affiliateUrl)}`, '_blank')} className="p-2 bg-green-500 hover:bg-green-600 text-white contenedorCosas transition-all" title="Compartir en WhatsApp">
